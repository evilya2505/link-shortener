import { FormValues, TLoginResult, TSqueezeObj, TSqueezeRequest, TUserInfo } from "./types";
...
class MainApi {
  ...
  private getTotalCountFromRes(res: Response): number {
    ...
    private _getRequestResult<T>(res: Response, includeTotalCount: boolean): Promise<{ result: T; totalCount?: number; }> {
  ...
  getStatistics(data: TSqueezeRequest): Promise<{ result: TSqueezeObj[], totalCount?: number }> {
    const queryParams = new URLSearchParams({
        order: data.order || 'desc_counter',
      offset: data.offset !== undefined ? data.offset.toString() : "0",
      limit: data.limit !== undefined ? data.limit.toString() : "1",
    }).toString();
    console.log(`${this._baseUrl}/api/statistics?${queryParams}`);
    return fetch(`${this._baseUrl}/api/statistics?${queryParams}`, {
      method: "GET",
      headers: {
        ...this._headers,
        Authorization: `Bearer ${localStorage.getItem("access_token")}`,
