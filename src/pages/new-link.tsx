...
const NewLinkPage: React.FC = (): JSX.Element => {
    ...
    try {
      if (lastResult?.short !== undefined) {
        await navigator.clipboard.writeText(
          `https://front-test.hex.team/s/${lastResult.short}`
        );
  ...
  return (
    <section className={newLinkStyles.section}>
      ...
      <div className={newLinkStyles.container}>
        <h2 className={newLinkStyles.containerTitle}>
          Создание короткой ссылки
        </h2>
        <form
          className={newLinkStyles.form}
        ...
        >
          <fieldset className={newLinkStyles.fieldset}>
            ...
            <button
              disabled={!isValid}
              className={newLinkStyles.button}
              type="submit"
            >
              Создать ссылку
            </button>
          ...
          {errors?.link && (
            <label className={newLinkStyles.error}>
              {errors?.link.message}
            </label>
          )}
        </form>
        <div
          className={`${newLinkStyles.resultContainer} ${
            lastResult && newLinkStyles.resultContainerVisible
          }`}
        >
          <p className={newLinkStyles.resultText}>{`${
            lastResult
              ? `https://front-test.hex.team/s/${lastResult.short}`
              : "Ссылка"
          }`}</p>
          <button
            ...
            className={newLinkStyles.resultButton}
          >
            <img
              className={newLinkStyles.buttonIcon}
              src={copyIcon}
              alt="скопировать"
            />
          </button>
          <div
            className={`${newLinkStyles.notification} ${
              notificationVisible && newLinkStyles.notificationVisible
            }`}
          >
            Ссылка скопирована.
          </div>
